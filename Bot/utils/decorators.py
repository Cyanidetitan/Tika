...
def ffmpeg_settings(id, input, FT):
    ...
    if audio_type == 'opus':
        ...
        if resolutio == '480p':
            ...
            cmd = f'ffmpeg -hide_banner -loglevel quiet -progress progress-{FT}.txt -i "{input}" -filter_complex "[0:v]drawtext=fontfile=font.ttf:text='t.me/animxt':fontsize=18:fontcolor=ffffff:alpha='if(lt(t,0),0,if(lt(t,5),(t-0)/5,if(lt(t,15),1,if(lt(t,20),(5-(t-15))/5,0))))':x=w-text_w-15:y=15" -preset {preset} -vcodec {vcodecs} -crf {crf} -s {resolution} -metadata title="Encoded By Animxt" -metadata:s:v title="Animxt - {resolutio} - {vcodec}"  -metadata:s:a title="Animxt" -map 0:v -c:a libopus -b:a 96k -map 0:a -c:s copy -map 0:s? "{output}" -y'
        else:
